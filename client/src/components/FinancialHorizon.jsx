    ...
    <div className="glass-card" style={{ padding: '12px 16px', fontSize: '0.85rem' }}>
      ...
      {payload.map((p, i) => (
        <div key={i} style={{ color: p.color || 'var(--text-primary)', fontWeight: 600, display: 'flex', justifyContent: 'space-between', gap: '16px' }}>
          ...
          <span>â‚¹{p.value.toLocaleString()}</span>
        </div>
      ))}
    </div>
  );
...
export default function FinancialHorizon() {
  return (
    ...
    >
      ...
      <div style={{ position: 'absolute', top: 0, left: '20%', width: '150px', height: '2px', background: '#ff0000', filter: 'blur(2px)' }} />
      <div style={{ position: 'absolute', bottom: 0, right: '20%', width: '150px', height: '2px', background: '#ff0000', filter: 'blur(2px)' }} />
      
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          <h3 style={{ fontFamily: 'var(--font-display)', color: '#ff0000', letterSpacing: '1px' }}>Financial Horizon</h3>
        ...
        <div style={{ textAlign: 'right' }}>
          ...
          <div style={{ color: 'var(--warning)', fontWeight: 600, fontFamily: 'var(--font-display)' }}>-â‚¹200.00</div>
        </div>
      </div>
